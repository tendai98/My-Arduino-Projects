#define SERVER_URL_URL "http://192.168.92.1/sensor"
...
void httpGet(char * url, char * bufferPtr){
  ...
  if(errorCode > 0){
     ...
     level = ((returnData.toInt()*100) / MAX_SAMPLE_VALUE); // Change the value to int and perform mathmetial operation
     returnData = String(level) + String("%");  // Set the value with unit
